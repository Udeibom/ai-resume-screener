...
async def upload_resume(
    ...
    try:
        ...
        resume_record = await pipeline.process_and_save_resume(
...
async def match_candidates(payload: MatchRequest, db: AsyncSession = Depends(get_db)):
    try:
        # Execute the RAG and LLM grading workflow
        engine = CandidateRankingEngine(db)
        rankings = await engine.rank_candidates(
            job_description=payload.job_description, limit_candidates=payload.limit
        )

        return MatchResponse(job_description=payload.job_description, results=rankings)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
